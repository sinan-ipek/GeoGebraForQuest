...
public:
    ...
    void Initialize() {
        ...
        baseTexture_.Initialize(device_.Get());

        Log("GeoGebraForQuest PC v0.11 initialized: CEF GPU texture -> OpenXR projection, no screen capture");
    }
...
private:
    ...
    SharedGpuTextureConsumer baseTexture_;
    std::int64_t gpuSequence_{};
    GpuFrameInfo gpuFrame_{};

    SharedSbsReader sbsReader_;
    ...
    void CreateInstance() {
        ...
        createInfo.applicationInfo.applicationVersion = 11;
        std::strcpy(createInfo.applicationInfo.engineName, "GGQ OpenXR");
        createInfo.applicationInfo.engineVersion = 11;
    ...
    void CreateProjectionSwapchain() {
        ...
        Log("Projection swapchain=" + std::to_string(width) + "x" +
    ...
    bool MakeStereoRect(const PanelRect& base, PanelRect& stereo) const {
        if (!sbsFrame_.active ||
            ...
            sbsFrame_.panelWidth < 2 || sbsFrame_.panelHeight < 2) {
            return false;
        }
        ...
        const float leftN = std::clamp(
            sbsFrame_.panelLeft / clientWidth, 0.0f, 1.0f);
        const float rightN = std::clamp(
            (sbsFrame_.panelLeft + sbsFrame_.panelWidth) / clientWidth,
            0.0f, 1.0f);
        const float topN = std::clamp(
            sbsFrame_.panelTop / clientHeight, 0.0f, 1.0f);
        const float bottomN = std::clamp(
            (sbsFrame_.panelTop + sbsFrame_.panelHeight) / clientHeight,
            0.0f, 1.0f);
    ...
    void RefreshSources() {
        ...
        if (gpuFrameReader_.ReadIfChanged(gpuSequence_, gpuUpdate)) {
            ...
            if (gpuFrame_.active) {
                try {
                    if (baseTexture_.Update(
                            device_.Get(), context_.Get(), gpuFrame_)) {
                        if ((gpuSequence_ % 120) == 0) {
                            Log("A GPU frame consumed seq=" +
                                std::to_string(gpuSequence_));
                        }
                    }
                } catch (const std::exception& ex) {
                    Log(std::string("A GPU update error: ") + ex.what());
                }
            }
        }

        SbsSnapshot sbsUpdate{};
        if (sbsReader_.ReadIfChanged(sbsSequence_, sbsUpdate)) {
            sbsSequence_ = sbsUpdate.sequence;
            sbsFrame_ = std::move(sbsUpdate);
            if (sbsFrame_.active && !sbsFrame_.sbs.empty()) {
                sbsTexture_.Upload(
                    device_.Get(),
                    context_.Get(),
                    sbsFrame_.sbs.data(),
...
int wmain(int argc, wchar_t** argv) {
    ...
    if (hostPid == 0) {
        MessageBoxW(
            ...
            L"GeoGebraForQuest PC v0.11",
            MB_OK | MB_ICONERROR);
        return 2;
    }

    try {
        Log("GeoGebraForQuest PC v0.11 XR starting, host pid=" +
