...
// Comfortable world-locked virtual monitor.
constexpr float kScreenWidthMeters = 1.65f;
constexpr float kScreenDistanceMeters = 1.55f;
constexpr float kStereoDistanceMeters = 1.53f; // B is 2 cm in front of A.
constexpr float kNearDepthMeters = 0.05f;
constexpr auto kDesktopCaptureInterval = std::chrono::milliseconds(33);
...
public:
    ...
    bool Capture(HWND hwnd, std::vector<std::uint8_t>& out, int& width, int& height) {
        ...
        if (width < 2 || height < 2 || !EnsureBitmap(width, height)) return false;
...
public:
    ...
    bool ReadIfChanged(std::int64_t previousSequence, SbsSnapshot& snapshot) {
        if (!view_ && !Open()) return false;

        for (int attempt = 0; attempt < 3; ++attempt) {
            const auto first = ReadI64(8);
            if ((first & 1) != 0) {
...
private:
    ...
    bool Open() {
        ...
        Log("Shared SBS mapping opened read-only safely");
        return true;
    }
    ...
    std::int64_t ReadI64(std::size_t offset) const {
        // Important: no InterlockedCompareExchange64 here. The mapping is FILE_MAP_READ.
        std::int64_t value{};
        std::memcpy(&value, view_ + offset, sizeof(value));
        return value;
    }
...
public:
    void Upload(
        ...
        if (!pixels || width < 2 || height < 2 || rowPitch < width * 4) return;
        if (!texture_ || width != width_ || height != height_) {
            ...
            CheckHr(device->CreateTexture2D(
                &desc, nullptr, texture_.ReleaseAndGetAddressOf()), "CreateTexture2D(source)");
            CheckHr(device->CreateShaderResourceView(
                texture_.Get(), nullptr, srv_.ReleaseAndGetAddressOf()), "CreateShaderResourceView(source)");
            width_ = width;
            height_ = height;
        }
...
PanelRect ScaleRect(const PanelRect& rect, float factor) {
    return {
        rect.left * factor,
        rect.right * factor,
        rect.top * factor,
...
float4 PSMain(VSOut i) : SV_TARGET { return tex0.Sample(samp0, i.uv); }
        ...
        Compile(shaderSource, "VSMain", "vs_5_0", vsBlob, errors);
        errors.Reset();
        Compile(shaderSource, "PSMain", "ps_5_0", psBlob, errors);

        CheckHr(device->CreateVertexShader(
            vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), nullptr,
            vertexShader_.ReleaseAndGetAddressOf()), "CreateVertexShader");
        CheckHr(device->CreatePixelShader(
            psBlob->GetBufferPointer(), psBlob->GetBufferSize(), nullptr,
            pixelShader_.ReleaseAndGetAddressOf()), "CreatePixelShader");
        ...
        CheckHr(device->CreateInputLayout(
            ...
            inputLayout_.ReleaseAndGetAddressOf()), "CreateInputLayout");
        ...
        CheckHr(device->CreateBuffer(&vb, nullptr, vertexBuffer_.ReleaseAndGetAddressOf()),
            "CreateBuffer(vertex)");
        ...
        CheckHr(device->CreateSamplerState(&sampler, sampler_.ReleaseAndGetAddressOf()),
            "CreateSamplerState");
        ...
        CheckHr(device->CreateRasterizerState(&raster, rasterizer_.ReleaseAndGetAddressOf()),
    ...
    void RenderEye(
        ...
        CheckHr(device->CreateRenderTargetView(
            target, &rtvDesc, rtv.ReleaseAndGetAddressOf()), "CreateRenderTargetView");
...
private:
    ...
    static void Compile(
        ...
        const HRESULT hr = D3DCompile(
            source, std::strlen(source), "GGQ-v0.10", nullptr, nullptr,
...
class RealStereoScreenApp {
public:
    explicit RealStereoScreenApp(DWORD hostPid) : hostPid_(hostPid) {}
    ...
    int Run() {
        ...
        Log("v0.10 real stereo screen initialized");
        Log("A distance=1.55m width=1.65m; B distance=1.53m");
        ...
        SbsSnapshot stereo{};
        std::int64_t lastReadSequence = -1;
        std::int64_t lastUploadedSequence = -1;
        std::int64_t lastLoggedSequence = -1;
            ...
            if (frameState.shouldRender) {
                ...
                if (hostWindow &&
                    (lastCapture.time_since_epoch().count() == 0 ||
                     now - lastCapture >= kDesktopCaptureInterval)) {
                    if (capture_.Capture(hostWindow, basePixels, baseWidth, baseHeight)) {
                        baseTexture_.Upload(
                            device_.Get(), context_.Get(), basePixels.data(),
                            baseWidth, baseHeight, baseWidth * 4);
                        lastCapture = now;
                    }
                }

                SbsSnapshot newest{};
                if (sharedSbs_.ReadIfChanged(lastReadSequence, newest)) {
                    lastReadSequence = newest.sequence;
                    stereo = std::move(newest);
                }

                const bool validSbs =
                    stereo.active &&
                    stereo.clientWidth > 1 && stereo.clientHeight > 1 &&
                    stereo.panelWidth > 1 && stereo.panelHeight > 1 &&
                    stereo.eyeWidth > 1 && stereo.eyeWidth <= kMaxEyeWidth &&
                    stereo.eyeHeight > 1 && stereo.eyeHeight <= kMaxEyeHeight &&
                    stereo.sbsStride == stereo.eyeWidth * 2 * 4 &&
                    !stereo.sbs.empty();

                if (validSbs && stereo.sequence != lastUploadedSequence) {
                    sbsTexture_.Upload(
                        device_.Get(), context_.Get(), stereo.sbs.data(),
                        stereo.eyeWidth * 2, stereo.eyeHeight, stereo.sbsStride);
                    lastUploadedSequence = stereo.sequence;
                }

                if (validSbs != lastStereoActive) {
                    lastStereoActive = validSbs;
                    Log(std::string("B stereo=") + (validSbs ? "ACTIVE" : "INACTIVE"));
                }
                ...
                CheckXr(xrLocateViews(
                    session_, &locate, &viewState, 2, &viewCount, views.data()),
                    "xrLocateViews");
                ...
                if (viewsUsable && baseTexture_.Valid() && baseWidth > 1 && baseHeight > 1) {
                    const std::uint32_t imageIndex = swapchain_.Acquire();
                    const float screenHeight =
                        kScreenWidthMeters * static_cast<float>(baseHeight) /
                    ...
                    if (validSbs && sbsTexture_.Valid()) {
                        const float l = std::clamp(
                            stereo.panelLeft / static_cast<float>(stereo.clientWidth), 0.0f, 1.0f);
                        const float r = std::clamp(
                            (stereo.panelLeft + stereo.panelWidth) /
                            static_cast<float>(stereo.clientWidth), 0.0f, 1.0f);
                        const float t = std::clamp(
                            stereo.panelTop / static_cast<float>(stereo.clientHeight), 0.0f, 1.0f);
                        const float b = std::clamp(
                            (stereo.panelTop + stereo.panelHeight) /
                            static_cast<float>(stereo.clientHeight), 0.0f, 1.0f);

                        PanelRect onA{
                            baseRect.left + l * kScreenWidthMeters,
                            baseRect.left + r * kScreenWidthMeters,
                            baseRect.top - t * screenHeight,
                            baseRect.top - b * screenHeight};

                        // Same apparent rectangle from the local origin while B sits 2 cm closer.
                        stereoRect = ScaleRect(
                            onA, kStereoDistanceMeters / kScreenDistanceMeters);
                        stereoRectPtr = &stereoRect;
                    }
                    ...
                    for (std::uint32_t eye = 0; eye < 2; ++eye) {
                        renderer_.RenderEye(
                            ...
                            views[eye], baseTexture_.Srv(), baseRect,
                    ...
                    if (validSbs && stereo.sequence != lastLoggedSequence) {
                        lastLoggedSequence = stereo.sequence;
                        Log(
                            "B frame seq=" + std::to_string(stereo.sequence) +
                            " frame=" + std::to_string(stereo.frameNumber) +
                            " panel=" + std::to_string(stereo.panelLeft) + "," +
...
private:
    ...
    StereoScreenRenderer renderer_;
    ...
    void InitializeOpenXr() {
        ...
        std::strncpy(create.applicationInfo.applicationName,
            "GeoGebraForQuest PC", XR_MAX_APPLICATION_NAME_SIZE - 1);
        create.applicationInfo.applicationVersion = 10;
        std::strncpy(create.applicationInfo.engineName,
            "GGQ-PC-v0.10-RealStereoScreen", XR_MAX_ENGINE_NAME_SIZE - 1);
        create.applicationInfo.engineVersion = 10;
        ...
        CheckXr(xrEnumerateEnvironmentBlendModes(
            ...
            0, &blendCount, nullptr), "xrEnumerateEnvironmentBlendModes(count)");
        std::vector<XrEnvironmentBlendMode> modes(blendCount);
        CheckXr(xrEnumerateEnvironmentBlendModes(
            instance_, systemId_, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
            blendCount, &blendCount, modes.data()), "xrEnumerateEnvironmentBlendModes");
        if (!modes.empty()) {
            blendMode_ =
    ...
    void InitializeD3D11() {
        ...
        for (UINT i = 0;; ++i) {
            ...
            if (factory->EnumAdapters1(i, adapter.ReleaseAndGetAddressOf()) ==
                DXGI_ERROR_NOT_FOUND) break;
            DXGI_ADAPTER_DESC1 desc{};
            adapter->GetDesc1(&desc);
            if (LuidEqual(desc.AdapterLuid, req.adapterLuid)) {
                selected = adapter;
                break;
            }
        }
        ...
        CheckHr(D3D11CreateDevice(
            ...
            device_.ReleaseAndGetAddressOf(), &obtained,
            context_.ReleaseAndGetAddressOf()), "D3D11CreateDevice");
        if (obtained < req.minFeatureLevel) {
            throw std::runtime_error("D3D feature level below OpenXR minimum");
        }
    }
    ...
    void CreateSessionAndSwapchain() {
        ...
        CheckXr(xrEnumerateViewConfigurationViews(
            instance_, systemId_, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
            0, &viewCount, nullptr), "xrEnumerateViewConfigurationViews(count)");
        if (viewCount != 2) throw std::runtime_error("PRIMARY_STEREO view count != 2");
        std::array<XrViewConfigurationView, 2> configs{
            XrViewConfigurationView{XR_TYPE_VIEW_CONFIGURATION_VIEW},
            XrViewConfigurationView{XR_TYPE_VIEW_CONFIGURATION_VIEW}};
        CheckXr(xrEnumerateViewConfigurationViews(
            instance_, systemId_, XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO,
            2, &viewCount, configs.data()), "xrEnumerateViewConfigurationViews");
        const int width = static_cast<int>(std::max(
            configs[0].recommendedImageRectWidth,
            configs[1].recommendedImageRectWidth));
        const int height = static_cast<int>(std::max(
            configs[0].recommendedImageRectHeight,
            configs[1].recommendedImageRectHeight));
        swapchain_.Create(session_, swapchainFormat_, width, height);
        Log("Projection swapchain=" + std::to_string(width) + "x" + std::to_string(height));
    }
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        Log("GeoGebraForQuest PC v0.10 real stereo screen starting, host pid=" +
            std::to_string(pid));
        RealStereoScreenApp app(pid);
        return app.Run();
