...
# Remove brittle wrapper identity gate: CefSharp can surface a different managed
# IWebBrowser wrapper during popup teardown even though it is the same native popup.
old_gate = '''            if (_authPopupBrowser is not null &&\n                !ReferenceEquals(_authPopupBrowser, popupWebBrowser)) return;\n\n'''
if old_gate not in t:
    raise SystemExit('v0.13.9 auth wrapper identity gate not found')
t = t.replace(old_gate, '', 1)

# Replace the popup-close body with step-by-step restore + root reload using the
# shared RequestContext. Reloading the local GeoGebra page makes the freshly
# authenticated cookies/session visible to the app immediately.
